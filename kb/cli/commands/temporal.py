...
from ..ui import console, print_success, print_error, print_info, print_entries_table, print_entry
...
def compare(period1_start, period1_end, period2_start, period2_end):
    ...
    try:
        ...
        if changes["lost_projects"]:
            console.print(
                f"[bold red]Lost projects:[/bold red] {', '.join(changes['lost_projects'])}"
            )
...
def evolution(topic, tag):
    ...
    try:
        with db.session_scope() as session:
            ...
            entry_service = EntryService(session, FileManager())
...
def growth(interval):
    ...
    try:
        ...
        console.print(
            f"  Total growth: {last['cumulative_entries']} entries, {last['cumulative_words']:,} words"
        )
